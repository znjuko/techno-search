...
std::vector<Point> GraphConverter::getBasePoints(const std::vector<Point> &leftPoints,
                                                 const std::vector<Point> &rightPoints)
{
...
std::vector<Point> GraphConverter::getFeaturesCenters(const double &middleX)
{
    ...
    std::sort(intersectedPoints.begin(), intersectedPoints.end(),
              [](const Point &p1, const Point &p2) { return p1.y <= p2.y; });
...
std::vector<std::vector<double>> GraphConverter::GetAdjacencyTableFromPoints(const std::vector<Point> &points)
{
    ...
    for (size_t i = 0; i < matrixSize; ++i) adjacencyTable[i].resize(matrixSize);

    for (size_t i = 0; i < matrixSize; ++i) adjacencyTable[0][i] = double(i);

    for (size_t i = 0; i < matrixSize; ++i) adjacencyTable[i][0] = double(i);
        ...
        while ((currentX == rightX && rightIndex < points.size()) || (lastX && rightIndex < points.size()))
        {
            ...
            for (auto feature : features)
            {
                ...
                if (!interactions.empty())
                {
                    ...
                    {
                        if ((point->x <= std::max(rightPoint.x, leftPoint.x) &&
                             point->x >= std::min(rightPoint.x, leftPoint.x) &&
                             point->y <= std::max(rightPoint.y, leftPoint.y) &&
                             point->y >= std::min(rightPoint.y, leftPoint.y)))
                        {
